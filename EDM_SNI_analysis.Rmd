...
#ggplot theme to apply to plots later
...
Multivariate models have the same structure, except instead of using single variables to reconstruct the attractors, we use contemporaneous values of multiple variables [@Deyle2016]. Instead of library vectors or points in state-space taking the form of, for example, $<x_t,x_{t-1},x_{t-2}>$, they now are formed in true multivariate space, e.g. $<Kelp_{t},Urchin_{t},Nutrients_{t}>$. Using the normalized time series, we will build library vectors that each include the variable we are trying to predict, as well as other causal variables illuminated by our CCM analysis. Then we use S-maps as above to forecast the target variable using leave-one-out cross-validation just as in Step 3.
...
```{r igraph, echo=F, message=F,warning=F}
...
#vertex coordinates
crds <- cbind(c(300,240,375,313,158,122,430,203,146.3342,309,20),c(10,126,102,206,204,85,162.8175,0,314.3077,318,232.6699))

# vector of node colors for plotting
vert.cols <- c(rep("tan3",3),rep("orchid3",2),"tan3",rep("gray70",5))

plot(g,asp=0.8, vertex.label.color="black", vertex.label.cex=0.8, edge.arrow.size=0.6,vertex.label.font=2, vertex.size=40,edge.lty=2,edge.color="black",vertex.color=vert.cols,layout=crds,vertex.frame.color=F,main="",edge.curved=F)
```

In this network, each node represents a variable, and the numbers associated with each are the number of causal connections in the web, outgoing and (incoming). The maximum number of outgoing interactions is 5 (6 total biological variables minus the variable itself), and incoming is 10 (5 biological and 5 physical variables). Algae species are in brown, while the two urchin species are in purple and the physical driver are in grey. For example, as mentioned above, we can see that young *Macrocystis* is forced by all other variables in the system, but only itself shows a causal forcing on 3 of the 5 other biological variables. Again we can see that while the web is not entirely saturated, there are many unidirectional forcings, and a lot of bidirectional causation as well (A forces B *and* B forces A).
...
We can pull from the models every predicted interaction between species. These fitted interaction strengths are specifically the estimated effect of the density of one species on itself or another species, when forecasting one monitoring period (approximately six months) ahead. For example, each data point for *Pterygophora* in the *Macrocystis* model is the predicted effect of the current density of Pterygophora on the density of Macrocystis in the next monitoring period. Mathematically, each interaction measured is a partial derivative, or element of our sequentially calculated Jacobian matrices, estimating the effect of one species on another, $\delta N_{1}/\delta N_{2}$. The distributions of those interactions are shown in the figure below.
...
With 10%/90% quantile regression. Columns/x-axis indicates the level of a physical stressor, while rows and y-axis denote the type of species interaction (corresponding to the colors in the boxplots above).

```{r individual_effects,fig.height=6}

phys_effects_on_interactions <- purrr::map(as.list(study_spp),function(spp) {
  quants <- c(0.10,0.5,0.90)
  ...
  ggplot(aes(ext_value,value,col=factor(effect,levels=c("mac","ymac","pter","lam","purp","red"))))+
    ...
    geom_hline(yintercept = 0,linetype=2)+
    geom_quantile(quantiles=quants,lineend = "round",size=1.5,formula=y~x)+
    scale_color_manual(values=c('mac'="navyblue",'ymac'="gray50",'pter'="darkgreen",'lam'="darkcyan",'purp'="mediumpurple4",'red'="darkred"),name="effect")+
...
* *Laminaria farlowii*
    + as far as competition goes, most large, negative interactions are associated with juvenile *Macrocystis*, likely as an indicator of impending succesional processes (i.e., the juvenile *Macrocystis* growing into large *Macrocystis*). This expected negative effect is reduced under elevated PDO, disturbance, and SST, which makes sense: these are conditions that classically inhibit *Macrocystis* dominance[@Reed2011; @Cavanaugh2011; @Bell2015,@Young2015].
...
* *Pterygophora californica*
    + Algal effects: *Laminaria* is a good indicator of a positive density response of *Pterygophora*, while, as mentioned above (and surprisingly) the effect of *Macrocystis* is small. The positive effect of *Laminaria* increases with slightly with elevated NPGO and disturbance, and decreases with elevated MEI, PDO, and SST.
    + Herbivory effects on *Pterygophora* vary with physical environment. *M. franciscanus* and *S. purpuratus* have similar mean effects on *Pterygophora*, but the effect of *M. franciscanus* is much more sensitive to physical conditions. Its negative effect is strongest under elevated disturbance and NPGO, while its effect is diminished under elevated MEI, PDO, and SST. The same trends hold for the effect of *S. purpuratus* on *Pterygophora*, but those effects are less sensitive.
    + The variation in the intraspecies effect of *Pterygophora* is responsive to physical conditions in a similar manner to *Laminaria* (reduced under elevated MEI, PDO, and SST, and increased under elevated NPGO and disturbance)
...
* **Temperature**
    + Elevated SST has similar effects to nutrient stress (elevated MEI and PDO, and reduced NPGO), which makes sense: In Southern California, elevated SST is associated with reduced nutrient availability[@Zimmerman1984,@Bell2015]. Similar effects as described above for nutrient stress hold for elevated temperature.
