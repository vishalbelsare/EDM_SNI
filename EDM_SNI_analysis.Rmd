...
all.mod.coeffs.plot.box <- all.mod.coeffs.long %>%
  ...
  geom_text(aes(x=1.2,y=1.8,label=label),size=6, parse=T,check_overlap=TRUE,show.legend = FALSE)+
  ylim(-2,2)+
  coord_flip()+
...
# ggsave("all_spp_coffs.png",plot=all.mod.coeffs.box,height=8,width=7.5)
```
...
```{r phys coeffs,echo=F,message=F,warning=F}
# with boxplot
...
We can observe this in our data by plotting the same interactions from the previous step against levels of the environmental variables. We will investigate the effect of environmental variables on mean interaction strength for the different interaction types (herbivory, competition, consumption).

```{r mean interactions, echo=F,warning=F,message=F}
# Mean interactions over time
mean_ints_by_time <- all.mod.coeffs.long %>%
  select(period,mod,effect,value,type,spp_type) %>%
...
#   ggplot(aes(x=period,y=meanint,ymax=upperint,ymin=lowerint,color=type))+
#   geom_line()+
#   geom_hline(yintercept=0,linetype=2)+
#   scale_color_manual(values=c("darkorange1","orangered2","darkorchid1","palegreen3","skyblue1"),name="Interaction\ntype")+
#   coord_cartesian(ylim=c(-0.5,0.5))+
#   labs(title="Mean species interactions\nby type over time",x="Monitoring Period",y="Interaction Strength")+
#   theme_rockwell()
...
all.coeffs.with.phys<- westend %>%
  ...
  distinct() %>%
  right_join(all.mod.coeffs.long,by="period")%>%
  gather("external_forcing","ext_value",one_of(phys.vars))

all.coeffs.with.phys %>%
  filter(!is.na(type))%>%
  ggplot(aes(ext_value,value))+
  geom_point()+
  geom_smooth()+
  ylim(-2,2)+
  labs(title="Physical forcing effect on species interactions",x="Normalized Index Value",y="Interaction Strength")+
  facet_grid(type~external_forcing)+
  theme_rockwell()

# absolute value of interactions
all.coeffs.with.phys %>%
  filter(!is.na(type))%>%
  mutate(absvalue=abs(value))%>%
  ggplot(aes(ext_value,absvalue))+
  geom_point()+
  geom_smooth()+
  labs(title="Physical forcing effect on species interactions",x="Normalized Index Value",y="Interaction Strength")+
  facet_grid(type~external_forcing)+
