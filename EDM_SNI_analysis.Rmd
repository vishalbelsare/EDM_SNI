...
```{r physical variables timeseries}
...
temp <- ccm(westend,lib=as.matrix(westend.segs),pred=as.matrix(westend.segs),E=tempE,lib_column= 'mac',target_column = 'purp',lib_sizes = c(10,25,50,75,100,125,150,200,300,400,500),num_samples=100,replace=T,silent=T,RNGseed = 41389)

mac_xmap_purp <- temp %>%
  group_by(lib_size)%>%
  summarise(rhomean=mean(rho,na.rm=T),upper=rhomean+sd(rho,na.rm=T),lower=rhomean-sd(rho,na.rm=T))%>%
  ungroup()%>%
...
dev.copy(pdf,"plots/multi_attractor.pdf")
dev.off()
...
```{r some single plots}
# macrocystis, pterygophora, and temperature
quants<-c(0.05,0.5,0.95)
all.coeffs.with.phys %>%
  filter(type=="algal competition",mod%in% c('ymac','pter'),effect %in% c('mac','pter'),external_forcing=="sst")%>%
  ...
  ggplot(aes(ext_value,value,col=factor(effect,levels=c("mac","ymac","pter","lam","purp","red"))))+
    ...
    geom_hline(yintercept = 0,linetype=2,col="black")+
    geom_quantile(quantiles=quants,lineend = "round",size=1,formula=y~x)+
    scale_color_manual(values=c('mac'="lightblue",'ymac'="gray30",'pter'="red",'lam'="darkcyan",'purp'="mediumpurple4",'red'="darkred"),guide="none")+
    labs(x="Normalized SST",y="Interaction Strength")+
    facet_grid(~mod,scales = "free_y")+
    theme(plot.margin=unit(c(0.5, 0.5, 0.5, 0.5), units="line"))

# macrocystis, pterygophora, and NPGO
all.coeffs.with.phys %>%
  filter(type=="algal competition",mod%in% c('mac','pter'),effect %in% c('mac','pter'),external_forcing=="npgo")%>%
  ggplot(aes(ext_value,value,col=factor(effect,levels=c("mac","ymac","pter","lam","purp","red"))))+
  geom_linerange(aes(ymin=lower,ymax=upper),col='gray60')+ 
  geom_point(col='black',alpha=0.6)+
   geom_hline(yintercept = 0,linetype=2,col="black")+
   geom_quantile(quantiles=quants,lineend = "round",size=1,formula=y~x)+
   scale_color_manual(values=c('mac'="lightblue",'ymac'="gray30",'pter'="red",'lam'="darkcyan",'purp'="mediumpurple4",'red'="darkred"),guide="none")+
   labs(x="Normalized NPGO",y="Interaction Strength")+
   facet_grid(~mod,scales = "free_y")+
  theme(plot.margin=unit(c(0.5, 0.5, 0.5, 0.5), units="line"))

# macrocystis, pterygophora, herbivory, and NPGO
all.coeffs.with.phys %>%
  filter(type=="herbivory",mod%in% c('mac','pter'),effect %in% c('red','purp'),external_forcing=="npgo")%>%
  ggplot(aes(ext_value,value,col=factor(effect,levels=c("mac","ymac","pter","lam","purp","red"))))+
   geom_linerange(aes(ymin=lower,ymax=upper),col='gray60')+
    geom_point(col='black',alpha=0.6)+
   geom_hline(yintercept = 0,linetype=2,col="black")+
...
# macrocystis, pterygophora, and waves
all.coeffs.with.phys %>%
  filter(type=="algal competition",mod%in% c('mac','pter'),effect %in% c('mac','pter'),external_forcing=="waves")%>%
  ggplot(aes(ext_value,value,col=factor(effect,levels=c("mac","ymac","pter","lam","purp","red"))))+
  geom_linerange(aes(ymin=lower,ymax=upper),col='gray60')+
  geom_point(col='black',alpha=0.6)+
   geom_hline(yintercept = 0,linetype=2,col="black")+
   geom_quantile(quantiles=quants,lineend = "round",size=1,formula=y~x)+
   scale_color_manual(values=c('mac'="lightblue",'ymac'="gray30",'pter'="red",'lam'="darkcyan",'purp'="mediumpurple4",'red'="darkred"),guide="none")+
   labs(x="Normalized SWH",y="Interaction Strength")+
   facet_grid(~mod,scales = "free_y")+
...
### Effect of disturbance on *Macrocystis*
```{r disturbance and mac,fig.height=7,fig.width=5}
quants <- c(0.05,0.5,0.95)
waves.mac.pter <- westend %>%
  select(one_of(c("pter","lam","mac","period"))) %>%
  distinct() %>%
  right_join(all.mod.coeffs.long,by="period")%>%
  gather("external_spp","ext_value",one_of(c('pter','lam','mac')))%>%
  filter(effect%in% c('waves','pter','lam'),mod=='mac')%>%
  ggplot(aes(ext_value,value))+
  geom_point(col='gray60',alpha=0.6)+
  geom_hline(yintercept = 0,linetype=2)+
  geom_quantile(quantiles=quants,lineend = "round",size=1,formula=y~x)+
  labs(x="Normalized Density",y="Effect of SWH on Macrocystis")+
  facet_grid(external_spp~effect,scales="free")
waves.mac.pter
```

### Effect of Macrocystis on Laminaria/Pterygophora interaction
```{r disturbance and mac,fig.height=7,fig.width=5}
quants <- c(0.05,0.5,0.95)
mac.lam.pter <- westend %>%
  select(one_of(c("mac","period"))) %>%
  distinct() %>%
  right_join(all.mod.coeffs.long,by="period")%>%
  gather("external_spp","ext_value",mac)%>%
  filter(effect=='pter',mod=='lam')%>%
  ggplot(aes(ext_value,value))+
  geom_point(col='gray60',alpha=0.6)+
  geom_hline(yintercept = 0,linetype=2)+
  geom_quantile(quantiles=quants,lineend = "round",size=1,formula=y~x)+
  labs(x="Normalized Density",y="Effect of Pterygophora on Laminaria")
mac.lam.pter
```
### Effect of urchin density on Macrocystis/Pterygophora interaction

```{r urchin dens mac pter}
quants <- c(0.05,0.5,0.95)
urchin.mac.pter <- westend %>%
  select(one_of(c("red","period"))) %>%
  distinct() %>%
  mutate(time=row_number())%>%
  right_join(all.mod.coeffs.long,by=c("period",'time'))%>%
  gather("external_spp","ext_value",red)%>%
  filter(effect %in% c('pter','mac'),mod %in% c('pter','mac'),!is.na(ext_value))%>%
  ggplot(aes(ext_value,value))+
  geom_point(col='gray60',alpha=0.6)+
  geom_hline(yintercept = 0,linetype=2)+
  geom_quantile(quantiles=quants,lineend = "round",size=1,formula=y~x)+
  labs(x="Normalized Red Urchin Density",y="Interaction Strength")+
  facet_wrap(~mod)
urchin.mac.pter
```

### Do conditions affect average interaction strength?

```{r conditions and interaction strength}
# disturbance. top quartile vs. bottom quartile
all.coeffs.with.phys %>%
  mutate(av_value=abs(value))%>%
  filter(external_forcing=='waves', type=='algal competition', ext_value>=quantile(westend$pdo,0.75,na.rm=T)|ext_value<=quantile(westend$pdo,0.25,na.rm=T))%>%
  mutate(stress_level=ifelse(ext_value>=quantile(westend$pdo,0.75,na.rm=T),'high','low'))%>%
  ggplot(aes(log(value),..density..,fill=stress_level))+
...
int_mean_sd <-spp.only.mod.coeffs %>%
  ...
  summarise(mean_int=mean(value,na.rm=T),abs_mean=abs(mean_int),sd_int=sd(value,na.rm=T),n=n(),cv_int=sd_int/abs_mean*100,
...
int_mean_sd %>%
  ...
  group_by(type)%>%
  summarise(mean_str=mean(abs_mean),ci95=sd(abs_mean)/sqrt(n())*1.96)%>%
  ungroup()%>%
  ggplot(aes(type,mean_str))+
  geom_col()+
  geom_errorbar(aes(ymax=mean_str+ci95,ymin=mean_str-ci95),position='dodge',width=0.2)+
  labs(x='Interaction Type',y='Interaction Strength')

# scatterplot mean vs. sd (removing intraspecies interactions)
int_mean_sd %>%
  filter(mod != "ymac",effect != 'ymac')%>%
  ggplot(aes(abs_mean,sd_int))+
...
# CV calculated using the correction for sample size (cv*=1+1/4n * cv)
int_mean_sd %>%
  filter(mod != 'ymac',effect!='ymac')%>%
...
int_mean_sd %>%
  ...
  left_join(int_mean_sd,by=c("rev_effect"="effect","rev_mod"="mod"))%>%
  mutate(upper1=mean_int.x+sd_int.x,lower1=mean_int.x-sd_int.x,upper2=mean_int.y+sd_int.y,lower2=mean_int.y-sd_int.y)-> int_mean_sd2
...
interaction_names <- colnames(interactions.block)[3:29]
...
for(i in 1:n_row) {
  for(j in 1:n_col) {
      ...
      tempE= westend.bestE[col_names[j]]
      temp <- ccm(interactions.block,lib=as.matrix(westend.segs),pred=as.matrix(westend.segs),E=tempE,lib_column= 2+i,target_column = 27+j,
...
ints.multi.mods <- purrr::map(interaction_names,function(x) {
  smap_multi(interactions.block,westend.segs,x,ints.causal.vars[[x]])
})
