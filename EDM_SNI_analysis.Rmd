...
g %v% "labels" <- names
int_net<-ggnet2(g,arrow.size=8,arrow.gap=0.05,color = "type")+
  geom_point(aes(color = color), size = 12, alpha = 0.5) +
  geom_point(aes(color = color), size = 9)+
  geom_text(aes(label=label),family="Rockwell",size=4)+
  theme(panel.border = element_rect(colour="white",fill=NA))

ggsave("plots/int_net.png",plot=int_net)
int_net
```
...
ggplot(aes(value,..density..,fill=phys_high_low))+
  ...
  geom_vline(aes(xintercept=mean_int,color=phys_high_low))+
  scale_fill_manual(values=c("red","blue"),name="Physical\nForcing Value")+
  scale_color_manual(values=c("red","blue"))+
  guides(color="none")+
  labs(x="Effect of Macrocystis on Pterygophora",y="Probability Density")+
  facet_grid(rows=vars(external_forcing))

```

### Distribution of interaction strengths by type

```{r interactions density plot by type,fig.height=6}
# Density plot
interaction_distribution_by_type <- spp.only.mod.coeffs %>%
  ...
  geom_density(aes(fill=type),alpha=0.8,col=NA)+
  geom_vline(xintercept=0,linetype=2)+
  ...
  geom_text(aes(x=-2,y=0.5,label=type),hjust="inward",family="Rockwell",fontface="italic",check_overlap = TRUE,size=4,show.legend = FALSE)+
  facet_grid(rows=vars(type))+
  theme(strip.text = element_blank())

interaction_distribution_by_type
ggsave("plots/int_dist_by_type_density.png",plot=interaction_distribution_by_type,h=6)
```
