...
quants<-c(0.05,0.5,0.95)
all.coeffs.with.phys %>%
  filter(type=="algal competition",mod%in% c('mac','pter'),effect %in% c('mac','pter'),external_forcing=="sst")%>%
  
  ggplot(aes(ext_value,value,col=factor(effect,levels=c("mac","ymac","pter","lam","purp","red"))))+
    geom_linerange(aes(ymin=lower,ymax=upper),col='gray60')+
    geom_point(col='gray60',alpha=0.6)+
    geom_hline(yintercept = 0,linetype=2,col="white")+
    geom_quantile(quantiles=quants,lineend = "round",size=1,formula=y~x)+
...
all.coeffs.with.phys %>%
  ...
  filter(external_forcing=='pdo', type=='algal competition', ext_value>=quantile(westend$pdo,0.75,na.rm=T)|ext_value<=quantile(westend$pdo,0.25,na.rm=T))%>%
  mutate(stress_level=ifelse(ext_value>=quantile(westend$pdo,0.75,na.rm=T),'high','low'))%>%
  ggplot(aes(av_value,..density..,fill=stress_level))+
